...
def num_to_bits(x, base=defaultBase, numBits=None):
    if numBits and x>=(base**numBits):
        raise RuntimeError("Insufficient bits to hold %i with base %i and %i bits. Raise base or numBits." % (x, base, numBits))
    digits = []
    while x:
...
def char_to_bits(c, base=defaultBase, numBits=None, charMap=chars):
    try:
        ...
        print e
        raise RuntimeError("Unable to hold %c's position (%i) in charmap [%s] with base %i and %i bits. Raise base, numBits, or change charMap." % (c, charMap.find(c), charMap, base, numBits))
    ...
    # Otherwise, we need to turn it into a matrix
        ...
        if isHorizontal:
            ...
            if addSpaces:
                ...
                # Add spaces between the split sub-arrays
                returnMe = numpy.hstack([numpy.vstack([numpy.hstack([item,[0]]) for item in section]) for section in splitArray])[:,:-1]
